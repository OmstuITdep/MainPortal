...
//        }
    ...
    public static function getTotalInfoOfFilesByDisAndSemester($student, $numdoc, $discipline, $semester, $db_dip, $typeList)
    {
        $result = '';
        if ($student) {
        ...
        WHERE (TRIM(t.type) in ('other', 'work') AND t.fnpp in (" . implode(',', $fnpp) . ") AND t.disc = " . CMisc::str(strtoupper(str_replace('0x', '', $discipline))) . " AND t.semester = " . $semester . ")
         and datediff(now(), from_unixtime(unixdate)) < 800
        ...
        WHERE (TRIM(t.type) in ('other', 'work') AND t.fnpp in (" . implode(',', $fnpp) . ") AND t.disc = " . CMisc::str(strtoupper(str_replace('0x', '', $discipline))) . ") and t.semester is null
         and datediff(now(), from_unixtime(unixdate))  < 400
        ORDER BY field(t.state, 1, 0, 2);
        LIMIT 1
        ";
    ...
    public static function getListOfFilesByDisAndSemester($md5)
    {
        $result = '';
        if (isset(Yii::app()->session[$md5])) {
            ...
            if ($info['db_dip'] != '0x8000000000000000' && $info['db_dip']) {
                $sql = "SELECT t.id, t.name, t.state, t.comment, t.size
        FROM ecab.vkrfiles t
        ...
        WHERE (t.fnpp in( " . implode(', ', $info['studentFnpp']) . ") AND t.disc = " . CMisc::str(str_replace('0x', '', $info['discipline'])) . " AND t.semester = " . $info['semester'] .
                ...
                $sql = "SELECT t.id, t.name, t.state, t.comment, t.size
        FROM ecab.vkrfiles t
        WHERE  (t.fnpp in( " . implode(', ', $info['studentFnpp']) . ") AND t.disc = " . CMisc::str(str_replace('0x', '', $info['discipline'])) . "  and datediff(now(), from_unixtime(unixdate)) < 400)";
...
';
                foreach ($files as $one) {
                    ...
                    $result .= '<td>';
                    $result .= '<span id="tooltipId" rel="tooltip" title= "' . $tooltip . '" data-toggle="tooltip" >' .
                        CHtml::link($one['name'] . ' (' . round($one['size'] / 1024 / 1024, 2) . ' Мбайт)', array('downLoadWorkFile', 'id' => $one['id']), [
    ...
    public static function checkCWAccess($year, $semester)
    {
        ...
        if ($semester == 'осенний' &
            (($date['mon'] == 10 & $date['mday'] > 20)
                || ($date['mon'] == 11 & $date['mday'] < 8))) {
            return true;
        } elseif (($date['mon'] == 3 & $date['mday'] > 0) || ($date['mon'] == 4 & $date['mday'] < 10)) {
            return true;
        }
        return false;
    }
